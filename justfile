...
eval-package-coverage:
    uv run pytest tests/eval -o addopts="" \
      ...
      --cov-fail-under=80 -m "not eval_binding_bench" -q
...
eval-per-file-coverage:
    ...
    uv run pytest tests/eval -o addopts="" -m "not eval_binding_bench" \
    ...
    uv run python tools/check_per_file_coverage.py \
      ...
      --file src/git_cg/eval/cache_json.py \
...
eval-binding-coverage:
    ...
    uv run pytest tests/eval/binding -o addopts="" -m "not eval_binding_bench" \
      --cov=git_cg.eval.binding \
      --cov-branch \
      --cov-report=term-missing \
...
eval-binding-bench:
    ...
    uv run pytest tests/eval/binding/test_binder_cache.py -o addopts="" -m eval_binding_bench -q
