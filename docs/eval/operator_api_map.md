...
| `eval doctor` | command | public | canonical | Check local suite health (pins, metrics, fixtures). — Public CLI operator surface. |
| `eval dogfood` | command | public (dark-launch; hidden from regular help) | canonical (dark-launch) | Capture Lane C dogfood evidence for a candidate commit message. — Maintainer/operator surface; registered and callable, but hidden from regular `git-cg eval --help` (dark launch). |
| `eval encode-fixture` | command | public | canonical | Print stable identity hashes for one evaluation fixture. — Public CLI operator surface. |
...
| `eval export drain` | command | public | canonical | Drain the export queue through the Opik transport. — Public CLI operator surface. |
| `eval export retry` | command | public | canonical | Re-queue failed export rows for another drain attempt. — Public CLI operator surface. |
| `eval export status` | command | public | canonical | Show export-queue status (read-only, offline). — Public CLI operator surface. |
...
| `eval resume` | command | public | canonical | Continue an unfinished evaluation from a checkpoint. — Public CLI operator surface. |
...
| `eval review rollup` | command | public | registered | Roll up multi-rater advisory scores for review items. — Present on Typer tree; see help. |
| `eval review show` | command | public | registered | Show one local review-queue item. — Present on Typer tree; see help. |
| `eval run` | command | public | canonical | Run a local offline evaluation suite. — Public CLI operator surface. |
| `eval session` | group | public (group) | group | Inspect local commit sessions. — Nested Typer group (not invoked alone). |
| `eval session show` | command | public | canonical | Show one local commit session. — Public CLI operator surface. |
| `eval thread` | group | public (group) | group | Inspect local session threads. — Nested Typer group (not invoked alone). |
| `eval thread show` | command | public | canonical | Show one local session thread. — Public CLI operator surface. |
...
git-cg eval lab pins …
git-cg eval lab run …
git-cg eval lab status …
