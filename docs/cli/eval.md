...
* `git-cg eval checkpoint` — Local evaluation checkpoint inventory (read-only).
...
* `git-cg eval encode-fixture` — Print stable identity hashes for one evaluation fixture.
...
* `git-cg eval gc` — Purge stale acceptpath debris.
...
* `git-cg eval resume` — Continue an unfinished evaluation from a checkpoint.
* `git-cg eval review` — Local human review queue (advisory only).
* `git-cg eval run` — Run a local offline evaluation suite.
