...
- [ ] Establish a repeatable LLM evaluation system. It needs to test the same data "diff" each time to ensure deterministic tracking of commit generations. Track and compare outputs, potentially using Opik GEval, to determine if newer models/prompts are objectively better. Speed cannot be the only metric.
  > PARTIALLY COMPLETED: 2026-06-07. Note: Added eval_commit_message.py but deferred full integration.
