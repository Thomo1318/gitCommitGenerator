...
def init_sentry():
    ...
    Sentry setup can be disabled by setting ``GIT_CG_DISABLE_SENTRY`` to ``"1"``. When enabled, any captured git diff output in Sentry event contexts is scrubbed before sending.
    """
    ...
    def scrub_data(event, hint):
        ...
        """
        Scrub diff output from a Sentry event.
        
        Parameters:
        	event: The event payload to sanitise.
        	hint: Additional information supplied with the event.
        
        Returns:
        	The event payload with any git_cg diff output replaced by "[SCRUBBED]".
        """
