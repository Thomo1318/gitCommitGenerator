...
class SemVerImpact(str, enum.Enum):
...
class Commit(BaseModel):
    ...
    body: Optional[str] = Field(
        default=None,
        description="Detailed explanation of the changes. Explain the 'why' and 'how'."
    )
    breaking_change: bool = Field(
        default=False,
        description="Whether this commit introduces a backwards-incompatible breaking change."
    )
    breaking_change_description: Optional[str] = Field(
        default=None,
        description="Explanation of the breaking change. Required if breaking_change is true."
    )
                ...
                with open(sop_path, "r") as f:
                    ...
                    if not entry:
                        ...
                        if entry:
                            ...
                            raise ValueError(f"Emoji '{self.gitmoji}' is not in the GitOps SOP matrix. Please select a valid literal unicode emoji.")
                            
                    # 2. Assert cc_type matches the matrix exactly
                    if entry and self.cc_type.value != entry.get("cc_type"):
                        raise ValueError(f"Emoji / Type mismatch! According to the SOP matrix, the emoji '{self.gitmoji}' MUST be paired with the type '{entry.get('cc_type')}'. You used '{self.cc_type.value}'. Please correct either the emoji or the type to match the SOP.")
            except ValueError:
                raise
