...
def get_commits_since(tag: str) -> list[str]:
    try:
        if tag:
            log_output = subprocess.check_output(["git", "log", f"{tag}..HEAD", "--pretty=format:%s|%b"], stderr=subprocess.DEVNULL).decode("utf-8")
        else:
            log_output = subprocess.check_output(["git", "log", "--pretty=format:%s|%b"], stderr=subprocess.DEVNULL).decode("utf-8")
...
def inject_file_versions(files: list[str], bump_type: str, sop_data: dict, dry_run: bool, verbose: bool):
    ...
    strategies = sop_data.get("specifications_and_standards", {}).get("version_injection_matrix", {}).get("strategies", {})
    if not strategies:
        return
...
def execute_release(dry_run: bool, verbose: bool):
    ...
    # Generate Changelog
    taxonomy_rules = sop_data.get("changelog_generation_rules", {}).get("taxonomy", [])
    ...
    if not dry_run:
        ...
        console.print(f"[bold green]CHANGELOG.md updated.[/bold green]")
        
        # We don't automatically tag here as user requested to just prepare it
        console.print(f"[bold yellow]Files modified. Run `git add . && git commit -m '🔖 release: {new_tag}'` and `git tag {new_tag}` to finish.[/bold yellow]")
