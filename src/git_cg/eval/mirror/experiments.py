...
def _slug(token: str) -> str:
    """Lowercase filesystem/URL-safe slug; empty input becomes ``unknown``."""
    return _NAME_SAFE.sub("_", token.strip().lower()).strip("_") or "unknown"


def _harness_version() -> str:
    """Best-effort package version for ExperimentPins (null-safe, never raises)."""
    try:
...
class ExportGitShaError(ValueError):
    ...
    def __init__(self, message: str, *, error_class: str = "export_validation") -> None:
        """Experiment builder error with closed ``error_class`` (default validation)."""
...
def resolve_git_sha(repo_root: Any | None = None) -> str:
    """Best-effort short git SHA; returns ``UNRESOLVED_GIT_SHA`` when unresolvable.

    Never raises - unresolved identity is a soft signal. Use
    ``require_resolved_git_sha`` when a pin must fail closed.
    """
    try:
        proc = subprocess.run(
