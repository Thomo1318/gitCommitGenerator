...
class ExportPlanError(ValueError):
    ...
    def __init__(self, message: str, *, error_class: str = "export_validation") -> None:
        """Attach a closed ``error_class`` (default ``export_validation``)."""
...
class LayerAObjects:
    ...
    def from_mapping(cls, raw: Mapping[str, Any] | None) -> LayerAObjects:
        """Coerce a loose mapping into typed Layer-A objects.

        Accepts ``bundles``/``bundle`` and ``session_threads``/``sessions``
        singular-or-plural forms. Never invents train labels.
        """
        if raw is None:
            return cls()
...
class ExportPlanResult:
    ...
    def to_dict(self) -> dict[str, Any]:
        """Public plan view for CLI/tests. Always sets ``product_accept_blocked=False``."""
        return {
            "mode": self.mode,
    ...
    def as_mirror_result(self) -> MirrorResult:
        """Project this offline plan into the P0-7 ``MirrorResult`` surface."""
...
def _as_config(config: Mapping[str, Any] | None) -> dict[str, Any]:
    """Normalize optional config to a plain dict; default mode is ``off``."""
    if config is None:
...
def _project_name(config: Mapping[str, Any]) -> str:
    """Resolve the eval project lane (never silent Default Project)."""
...
def _normalize_layer_a(
    ...
    """Accept LayerAObjects, bare bundle, envelope mapping, or bundle sequence."""
