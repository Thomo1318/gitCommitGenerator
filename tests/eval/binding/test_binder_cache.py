...
The 1k/10k harness is marked ``eval_binding_bench`` and excluded from
default pytest. Run it with ``just eval-binding-bench``
(``-m eval_binding_bench``). It records bounded-scan and lock-hold
timings and does not ratify K or the lock budget.
...
@pytest.mark.eval_binding_bench
